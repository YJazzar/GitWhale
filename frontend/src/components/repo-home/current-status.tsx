...
export function CurrentStatus({ data }: CurrentStatusProps) {
	return (
		<Card>
			...
			<CardContent className="space-y-3 pt-0">
				...
				<div className="space-y-2">
					...
					<div className="p-2.5 bg-muted/50 rounded-lg space-y-1.5">
						<p className="text-sm font-medium leading-tight">{data.lastCommit.message}</p>
						<div className="flex items-center gap-3 text-xs text-muted-foreground">
							<CommitHash 
								commitHash={data.lastCommit.hash}
								repoPath=""
								shortHash={true}
								showIcon={false}
								enableCopyHash={true}
							...
							<div className="flex items-center gap-1">
								...
								<span>{data.lastCommit.date.toLocaleDateString()}</span>
							</div>
						</div>
					</div>
				</div>
